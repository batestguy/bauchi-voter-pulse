...
def validate_data():
    ...
    for name in ["needs.csv", "achievements.csv", "promises.csv"]:
...
def render():
    ...
    arrow_cards = []
    for sector in ["health", "education", "wash", "governance", "livelihoods", "security", "agriculture"]:
...
<section class="sources-section" id="sources"><div class="shell"><div class="section-head"><div><div class="eyebrow">{copy("Traceable by design", "An tsara shi don sa ido")}</div><h2>{copy("Every record has a source.", "Kowane bayana yana da sauro.")}</h2></div><p>{copy(f"{len(manifest_rows)} official pages archived. {len(pending_review_rows)} candidate records are queued for source review before they can become achievements.", f"An ruƙe shafi {len(manifest_rows)} na gwamnati. An sanya bayanan {len(pending_review_rows)} a cikin bita kafin su iya zama ayyuka.")}</p></div><ul class="source-list">{source_footer(sources)}</ul><div class="source-legend"><span><b>A</b> {copy("Primary or institutional record", "Bayanan gwamnati ko institucio")}</span><span><b>B</b> {copy("Programme or corroborating evidence", "Shirin ko tabbacin da ke tabbatar")}</span><span><b>D</b> {copy("Campaign material", "Kayan gaggawa")}</span></div></div></section>
</main>
...
const setLanguage=(lang)=>{{currentLanguage=lang;root.lang=lang;document.querySelectorAll('[data-en][data-ha]').forEach(el=>{{el.textContent=el.dataset[lang]||el.dataset.en}});document.querySelectorAll('[data-lang]').forEach(btn=>{{const active=btn.dataset.lang===lang;btn.classList.toggle('active',active);btn.setAttribute('aria-pressed',String(active));}});}};
...
document.querySelectorAll('[data-lga]').forEach(btn=>btn.addEventListener('click',()=>{{document.querySelectorAll('[data-lga]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');const lga=btn.dataset.lga;const summary=currentLanguage==='ha'?btn.dataset.summaryHa:btn.dataset.summary;const promise=currentLanguage==='ha'?btn.dataset.promiseHa:btn.dataset.promise;const result=currentLanguage==='ha'?btn.dataset.resultHa:btn.dataset.result;document.getElementById('selected-lga').textContent=lga;const copy=document.getElementById('selected-copy');copy.textContent=currentLanguage==='ha'?lga+': '+summary+' APM: '+promise+' Bari: '+result:lga+': '+summary+' APM: '+promise+' Next result: '+result;}}));
</script>
</body>
</html>'''
